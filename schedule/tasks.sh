...
update_leetcode_stats_command="curl -s -S -f -X POST -H 'Content-Type: application/json' -H 'Authorization: ${API_KEY}' -d '{\"command\": \"update_leetcode_stats\"}' \"$BASE_URL/admin/command/\""

# every 6 hours, offset by 3 hours
update_github_stats_schedule="0 3-23/6 * * *"
update_github_stats_command="curl -s -S -f -X POST -H 'Content-Type: application/json' -H 'Authorization: $API_KEY' -d '{\"command\": \"update_github_stats\"}' \"$BASE_URL/admin/command/\""
